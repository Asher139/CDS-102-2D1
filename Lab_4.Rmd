...
brauer_tidy <- brauer5 %>%
  separate(sample, into = c("nutrient","rate"), sep = 1, convert = TRUE) %>%
  mutate(across(c(gene_name, biological_process, molecular_function, systematic_id, number),
                str_trim))
...
brauer_tidy %>%
  ...
  ggplot(aes(x = rate, y = expression, color = nutrient, group = nutrient)) +
  geom_line() +
  geom_point() +
  labs(title = "Expression of LEU1 across growth rates",
       x = "Growth rate", y = "Expression level") +
  theme_minimal()
