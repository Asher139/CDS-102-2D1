---
title: "Lab 3: Wrangling sales data"
author: "MinJae Jo"
date: "`r Sys.Date()`"
documentclass: article
geometry: margin=1in
fontsize: 11pt
output:
  pdf_document:
    highlight: tango
    toc: false
    df_print: tibble
    fig_caption: no
    number_sections: no
    dev: pdf
  html_document:
    highlight: tango
    self_contained: true
    theme: paper
    toc: no
    df_print: kable
    fig_caption: no
    number_sections: no
    smart: yes
    dev: svg
  github_document:
    html_preview: false
    toc: true
    toc_depth: 4
    dev: svg
    df_print: kable
---

```{r setup, include = FALSE}
# DO NOT ALTER CODE IN THIS CHUNK
# Set knitr options
knitr::opts_chunk$set(
  echo = TRUE, eval = TRUE, fig.width = 5,
  fig.asp = 0.618, out.width = "80%", dpi = 120,
  fig.align = "center", cache = FALSE
)

# Load packages
suppressPackageStartupMessages(library(tidyverse))

# Load dataset
coffeeshop <- read_csv("coffeeshop.csv")
coffeeshop$date <- as.Date(coffeeshop$date)
```



### Exercise 1

# i.
select(coffeeshop, !cogs)
#!cogs → Select all columns except for the cogs column

# ii.
select(coffeeshop, starts_with("prod"))
#Starts_with("prod") → product_line, select a column that begins with "prod", such as product

# iii.
select(coffeeshop, contains("pe"))
#contains("pe") → Select total_expenses column with "pe" in column name

# iv.
select(coffeeshop, caffeination = type)
#Recall and rename the correction = type → type column


### Exercise 2

```{r, echo=FALSE}
# i. coffeeshop
coffeeshop %>%
  select(caffeination = type)
```
```{r, echo=FALSE}
# ii. coffeeshop
coffeeshop %>%
  select(starts_with("prod")) %>%
  filter(product == "Darjeeling")
```



### Exercise 3

```{r, echo=FALSE}
coffeeshop %>%
  mutate(product_profit = sales - (cogs + total_expenses))
```

```{r, echo=FALSE}
coffeeshop %>%
  mutate(sales - (cogs + total_expenses))
```

### Exercise 4

```{r, echo=FALSE}
coffeeshop %>%
  mutate(
    product_profit = sales - (cogs + total_expenses),
    expiration_date = date + 2
  )
```

### Exercise 5

```{r, echo=FALSE}
coffeeshop_updated <- mutate(
  coffeeshop,
  product_profit = sales - (cogs + total_expenses)
)
head(coffeeshop_updated, 10)
```


### Exercise 6

##Colombian
```{r, echo=FALSE}
coffeeshop_updated %>%
  group_by(product) %>%
  summarize(avg_profit = mean(product_profit, na.rm = TRUE))
```

##Central
```{r, echo=FALSE}
coffeeshop %>%
  group_by(market) %>%
  summarize(
    total_profit = sum(sales - (cogs + total_expenses), na.rm = TRUE),
    total_sales = sum(sales, na.rm = TRUE),
    profit_margin = total_profit / total_sales
  )
```

### Exercise 7

```{r, echo=FALSE}
coffeeshop %>%
  group_by(product_line, type) %>%
  summarize(
    total_profit = sum(sales - (cogs + total_expenses), na.rm = TRUE),
    total_sales = sum(sales, na.rm = TRUE),
    profit_margin = total_profit / total_sales
  )
```
##Exercise 6 is counted by market, and Exercise 7 is counted by product.
